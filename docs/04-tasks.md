...
| # | 단계 | 검증 방법 | 완료 |
|---|---|---|---|
| 1 | frontend 폴더와 index.html | `http://127.0.0.1:8000/` 에서 화면이 뜸 | [x] |
| 2 | Tailwind CDN 과 테마 토글 | 토글 후 새로고침해도 유지 | [x] |
| 3 | 목록 카드 렌더 | 배지와 D-N HH:MM 표시 | [x] |
| 4 | 추가 폼 | 제출 시 목록에 추가됨 | [x] |
| 5 | 수정 모달 단건 조회 후 전 필드 | 모달에 전 필드가 채워지고 PUT 200 | [x] |
| 6 | 삭제 확인 | 확인 후 204, 목록에서 사라짐 | [x] |
| 7 | 360px 반응형 | 개발자도구 폭 360px 에서 안 깨짐 | [x] |
| 8 | API p95 200ms 측정 | `GET /api/tasks` 와 `GET /api/tasks/{id}` 를 각각 50회 호출해 p95 측정 | [x] |
